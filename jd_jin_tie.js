                ...
                if(err){
                    ...
                    if(data.resultCode === 0){
                        if(data.resultData.code === '0000'){
                            ...
                            $.recevieTask = $.taskData.filter(t => t.status === 0) || [];
                            console.log(`未完成的任务：${$.recevieTask.length} 个`)
                            await queryMissionReceiveAfterStatus_xh($.recevieTask)
                        } else {
...
function queryMissionReceiveAfterStatus_xh(taskArr){
    return new Promise((resolve) => {
        for(let i = 0; i < taskArr.length; i++){
            // const body = JSON.stringify(`{"missionId":"${taskArr[i].missionId}"}`)
            const body = JSON.stringify({ "missionId": `${taskArr[i].missionId}` })
            ...
            $.get(options, async(err, resp, data) => {
                try{
                    if(err){
                        ...
                        if(data.resultCode === 0){
                            if(data.resultData.code === '0000'){
                                console.log('任务领取成功，等待10s')
                                await finishReadMission_xh(`${taskArr[i].missionId}`)
                            } else if(data.resultData.code === '0003'){
                                console.log('任务领取失败', "非法参数")
                            } else if(data.resultData.code === '0001'){
                                console.log('任务领取失败', "状态不正确")
                            } else {
                                console.log("任务领取失败", data)
                            }
                        } else {
                            console.log('任务领取失败', data)
                        }
