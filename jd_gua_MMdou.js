...
入口：APP首页-领京豆-升级赚京豆
cron:31 8 * * *
31 8 * * * https://raw.githubusercontent.com/smiek2121/scripts/master/gua_MMdou.js, tag=MM领京豆, enabled=true
...
!(async () => {
  if (!cookiesArr[0]) {
    $.msg('【京东账号一】已失效', '【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获取', 'https://bean.m.jd.com/bean/signIndex.action', { "open-url": "https://bean.m.jd.com/bean/signIndex.action" });
    return
  }
...
function get_UA() {
  var uuid = Array.from({length: 40}, () => Math.floor(Math.random() * 36).toString(36)).join('');
  var addressid = Array.from({length: 10}, () => Math.floor(Math.random() * 10).toString()).join('');
  var iosVer = ["15.1.1", "14.5.1", "14.4", "14.3", "14.2", "14.1", "14.0.1"][Math.floor(Math.random() * 7)];
  var iosV = iosVer.replace('.', '_');
  var clientVersion = ["10.3.0", "10.2.7", "10.2.4"][Math.floor(Math.random() * 3)];
  var iPhone = ["8", "9", "10", "11", "12", "13"][Math.floor(Math.random() * 6)];
  var area = Array.from({length: 4}, () => Math.floor(Math.random() * 10).toString()).join('') + '_' + Array.from({length: 4}, () => Math.floor(Math.random() * 10).toString()).join('') + '_' + Array.from({length: 5}, () => Math.floor(Math.random() * 10).toString()).join('') + '_' + Array.from({length: 5}, () => Math.floor(Math.random() * 10).toString()).join('');
  var ADID = Array.from({length: 8}, () => Math.floor(Math.random() * 16).toString(16)).join('') + '-' + Array.from({length: 4}, () => Math.floor(Math.random() * 16).toString(16)).join('') + '-' + Array.from({length: 4}, () => Math.floor(Math.random() * 16).toString(16)).join('') + '-' + Array.from({length: 4}, () => Math.floor(Math.random() * 16).toString(16)).join('') + '-' + Array.from({length: 12}, () => Math.floor(Math.random() * 16).toString(16)).join('');
  var lng = '119.31991256596' + Math.floor(Math.random() * 899 + 100);
  var lat = '26.1187118976' + Math.floor(Math.random() * 899 + 100);
  $.UA = 'jdapp;iPhone;10.1.2;' + iosVer + ';' + uuid + ';network/wifi;model/iPhone' + iPhone + ',1;addressid/0;appBuild/167802;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS ' + iosV + ' like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1';
  return $.UA
}
