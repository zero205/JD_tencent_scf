...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://api.jdsharecode.xyz/api/jxcfd/30`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
...
function uploadShareCode(code) {
  ...
  console.log(pin)
  return new Promise(async resolve => {
    $.get({url: `https://api.jdsharecode.xyz/api/autoInsert/jxcfd?sharecode=${code}&pin=${pin}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
