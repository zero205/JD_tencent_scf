...
先内部助力，有剩余助力作者
 脚本没有自动开卡，会尝试领取开卡奖励
 cron 23 8,9 * * * zx_productZ4Brand.js
 一天要跑2次
 */
const $ = new Env('特务Z-以物之名');
...
!(async () => {
    ...
    authorCodes=['Sv_x3QBcZ_FzXIA','S5KkcN2tzlROMZFKo1oFv','S5KkcIkVnvQurXGeF6oNS','S5KkcG3dLiQOfdnmV765R','S5KkcRk1L8lLRckj1l6QKdg','S5KkcRRpKo1SCck-ikvdYIg','Sv_hzQhwZ8FbUIRib1A'];
...
async function main() {
    ...
    for (let i = 0; i < $.taskList.length; i++) {
        ...
        if (task.assignmentType === 2) {
            console.log(`助力码：${task.ext.assistTaskDetail.itemId}`);
            shareCodes.push(task.ext.assistTaskDetail.itemId)
            $.encryptAssignmentId=task.encryptAssignmentId;
            // $.code=task.ext.assistTaskDetail.itemId;
            // $.allInvite.push({
            //     'userName': $.UserName,
            //     'code': task.ext.assistTaskDetail.itemId,
            //     'time': 0,
...
async function doTask() {
    for (let i = 0; i < $.taskList.length; i++) {
        ...
        if ($.oneTask.assignmentType === 1 || $.oneTask.assignmentType === 0 || $.oneTask.assignmentType === 7 || $.oneTask.assignmentType === 5) {
            if ($.oneTask.assignmentType === 7) {
                console.log(`任务：${$.oneTask.assignmentName}，尝试领取开卡奖励；（不会自动开卡，如果你已经是会员，则会领取成功）`);
            } if ($.oneTask.assignmentType === 1) { //浏览

            } if ($.oneTask.assignmentType === 5) { //首页下滑
                continue;
...
async function takeRequest(type) {
    ...
    switch (type) {
        case 'showSecondFloorCardInfo':
            url = `https://api.m.jd.com/?uuid=8343365623035683-8336532356830363&client=wh5&area=0_0_0_0&appid=ProductZ4Brand&functionId=showSecondFloorCardInfo&t=${Date.now()}&body=%7B%22source%22:%22card%22%7D`;
            break;
        ...
        case 'superBrandDoTask':
            if ($.runInfo.itemId === null) {
                url = `https://api.m.jd.com/?uuid=2613663363562616-5626036323564666&client=wh5&area=1_0_0_0&appid=ProductZ4Brand&functionId=superBrandDoTask&t=${Date.now()}&body=%7B%22source%22:%22card%22,%22activityId%22:${$.activityId},%22encryptProjectId%22:%22${$.encryptProjectId}%22,%22encryptAssignmentId%22:%22${$.oneTask.encryptAssignmentId}%22,%22assignmentType%22:${$.oneTask.assignmentType},%22completionFlag%22:1,%22itemId%22:%22${$.runInfo.itemId}%22,%22actionType%22:0%7D`;
            } else {
