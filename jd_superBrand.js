...
[task_local]
#特物Z
30 11,20 * * * https://raw.githubusercontent.com/Wenmoux/scripts/wen/jd/jd_superBrand.js, tag=特物Z, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
            ...
            await getid("showSecondFloorPkInfo", "pk")
            if ($.cando && $.enpid) {
                await getCode("pk", $.actid)
                await superBrandPkJoinTeam("pk")
                // console.log($.taskList);
                if ($.taskList) {
                    for (task of $.taskList) {
                        if (task.assignmentType == 0) { // 分享任务 
                            await doTask("pk", $.enpid, task.encryptAssignmentId, "", 0)
                        } else if (task.assignmentType == 3 && task.ext.followShop) {  //关注店铺 
                            for (let vo of task.ext.followShop) {
                                await doTask("pk", $.enpid, task.encryptAssignmentId, vo.itemId, 3)
                            }
                        } else if (task.assignmentType == 1 && task.ext.productsInfo) {  //关注店铺 
                            for (let vo of task.ext.productsInfo) {
                                await doTask("pk", $.enpid, task.encryptAssignmentId, vo.itemId, 3)
                            }
                        } else if (task.assignmentType == 1 && task.ext.shoppingActivity) {  //关注店铺 
                            for (let vo of task.ext.shoppingActivity) {
                                await doTask("pk", $.enpid, task.encryptAssignmentId, vo.itemId, 3)
                            }
                        } else {
                            if (task.assignmentType == 7) {  //领取开卡奖励
                                for (let vo of task.ext.brandMemberList) {
                                    await doTask("pk", $.enpid, task.encryptAssignmentId, vo.itemId, 7)
                                }
                            }
                        }
                    }
                ...
                if (err) {
                    ...
                    if (data.data && data.code === "0" && data.data.result) {
                        ...
                        if (result.activityBaseInfo) {
                            ...
                            $.callNumber = result.activityPkInfo.userHotNum;
                ...
                if (err) {
                    ...
                    if (data && data.data && data.code === "0") {
                        if (data.data.result && data.data.result.taskList) {
                            $.taskList = data.data.result.taskList.filter(x => x.assignmentType == 3 || x.assignmentType == 7 || x.assignmentType == 0 || x.assignmentType == 5 || x.assignmentType == 1)
                            ...
                            $.allInvite.push({
                                'userName': $.UserName,
                                'code': result.ext.assistTaskDetail.itemId,
                                'time': 0,
                                'max': true
                            });
                        } else {
...
function doTask(source, pid, encryptAssignmentId, id, type) {
    return new Promise(async (resolve) => {
        ...
        if (type === 5) { body = `{"source":"${source}","activityId":${$.actid},"encryptProjectId":"${pid}","encryptAssignmentId":"${encryptAssignmentId}","assignmentType":${type},"dropDownChannel":1,"itemId":"${id}","actionType":0}` }
...
function superBrandPkJoinTeam(source) {
    return new Promise(async (resolve) => {
        body = `{"source":"${source}","activityId":${$.actid},"pre":"pre","teamName":"left"}`
        const options = taskPostUrl("superBrandPkJoinTeam", body)
        // console.log(options);
        $.post(options, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    //   console.log(data)
                    if (data && data.code === "0") {
                        console.log("加入" + data.data.bizMsg);
...
function taskPostUrl(functionid, body) {
    const time = Date.now();
    return {
        url: `https://api.m.jd.com/?functionId=${functionid}&appid=ProductZ4Brand&client=wh5&t=${time}&body=${encodeURI(body)}`,
        body: "",
        headers: {
